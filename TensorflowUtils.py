...
def get_model_data(dir_path, filename):
...
'''
def weight_variable(shape, stddev=0.02, name):
    # print(shape)
    initial = tf.truncated_normal(shape, stddev=stddev)
    assert name is not None, "name is None" # name shouldn't be None
...
def bottleneck_unit(x, res_param, branch1_w, branch1_b, branch2a_w, branch2a_b, branch2b_w, branch2b_b, branch2c_w, branch2c_b, out_chan1, out_chan2, down_stride=False, up_stride=False, name=None):
    """
    ...
    if in_chans == out_chan2:
        ...
        first_stride = res_param[1]
        b1 = tf.nn.conv2d(x, branch1_w, strides=[1, first_stride, first_stride, 1], padding='VALID', name='conv')
        #b1 = bn(b1, 'bn%s_branch1' % name, 'scale%s_branch1' % name)
        #b1 = tf.nn.bias_add(b1, branch1_b)
        b1 = bn(b1, 'bn%s_branch1' % name)
        #b1 = tf.nn.bias_add(b1, branch1_b)
    b2 = tf.nn.conv2d(x, branch2a_w, strides=[1,first_stride,first_stride,1], padding='VALID', name='conv')
    #b2 = tf.nn.bias_add(b2, branch2a_b)
    b2 = bn(b2, 'bn%s_branch2a' % name)
    #b2 = tf.nn.bias_add(b2, branch2a_b)
    b2 = tf.nn.relu(b2, name='relu')

    b2 = tf.nn.conv2d(b2, branch2b_w, strides=[1,1,1,1], padding='SAME', name='conv')
    #b2 = tf.nn.bias_add(b2, branch2b_b)
    b2 = bn(b2, 'bn%s_branch2b' % name)
    #b2 = tf.nn.bias_add(b2, branch2b_b)
    b2 = tf.nn.relu(b2, name='relu')

    b2 = tf.nn.conv2d(b2, branch2c_w, strides=[1,1,1,1], padding='VALID', name='conv')
    #b2 = tf.nn.bias_add(b2, branch2c_b)
